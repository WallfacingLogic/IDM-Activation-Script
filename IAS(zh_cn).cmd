...
::   IDM 激活脚本 (IAS)
::
::   主页: https://github.com/hanmaoye/IDM-Activation-Script
::
::       邮箱: hanmaoye@qq.com
::
::============================================================================
...
::  要激活，请使用 "/act" 参数运行脚本，或在下面一行中将 0 改为 1
set _activate=0

::  要冻结30天试用期，请使用 "/frz" 参数运行脚本，或在下面一行中将 0 改为 1
set _freeze=0

::  要重置激活和试用，请使用 "/res" 参数运行脚本，或在下面一行中将 0 改为 1
set _reset=0
...
:: 如果在64位Windows上由32位进程启动，则使用64位进程重新启动脚本
:: 或在ARM64 Windows上由x86/ARM32进程启动时使用ARM64进程重新启动脚本
...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 错误：脚本存在LF行结束符问题，或者脚本末尾缺少空行。
echo:
...
echo [0] 继续使用
echo:
call :_color %_Green% "请输入键盘菜单选项 [1,0] :"
choice /C:10 /N
if !errorlevel!==2 rem
...
::  以下代码也适用于ARM64 Windows 10（包括x64位仿真）
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "请输入键盘菜单选项 [1,2,3,4,5,0]"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo:
echo 正在创建CLSID注册表项备份到 %SystemRoot%\Temp
...
echo %line%
echo:
echo      激活功能对某些用户不起作用，IDM可能会显示虚假序列号提示窗口。
echo:
call :_color2 %_White% "     " %_Green% "建议改用冻结试用期选项。"
echo %line%
echo:
...
:: 通过 ping internetdownloadmanager.com 和端口80测试检查网络连接
...
echo:
call :_color %Gray% "如果出现虚假序列号窗口，请改用冻结试用期选项。"
) else (
call :_color %Green% "IDM 30天试用期已成功永久冻结。"
echo:
call :_color %Gray% "如果IDM显示注册弹窗，请重新安装IDM。"
)
...
foreach ($regPath in $regPaths) {
	...
	Write-Host
	Write-Host "正在 $regPath 中搜索 IDM CLSID 注册表项"
	Write-Host
    ...
    foreach ($key in $filteredKeys) {
        ...
        if (($defaultValue -match "^\d+$") -and ($key.SubKeyCount -eq 0)) {
            ...
            Write-Output "$($key.PSChildName) - 在默认值中找到数字且无子项"
            continue
        }
        if (($defaultValue -match "\+|=") -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 在默认值中找到 + 或 = 且无子项"
            continue
        }
        ...
        if (($versionValue -match "^\d+$") -and ($key.SubKeyCount -eq 1)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 在 \Version 中找到数字且无其他子项"
            continue
        }
        ...
        if (($key.ValueCount -eq 0) -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 找到空项"
            continue
        }
    }
}
...
if (($finalValues.Count -gt 20) -and ($toggle -ne $null)) {
	$lockKey = $null
	$deleteKey = 1
    Write-Host "IDM 密钥数量超过20个。现在将删除它们而不是锁定..."
	Write-Host
}
